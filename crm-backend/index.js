...
function makeTodoItemFromData(data) {
  ...
  // if (!todoItem.fio) errors.push({ field: 'fio', message: 'Не указано fio' });
...
function updateTodoItem(itemId, data) {
  ...
  const itemIndex = todoItems.findIndex(({ id }) => id === Number(itemId));
  if (itemIndex === -1) throw new TodoApiError(404, { message: 'Client Not Found' });
